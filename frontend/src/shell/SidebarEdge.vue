<!--
  ...
  reopen. Ported from CRM's frontend2; positioned against the panel's wrapper, not the shell.
-->
<template>
	<div
...
function onPointerMove(event: PointerEvent) {
	...
	const passes = props.open ? distance <= -DRAG_DISTANCE : distance >= DRAG_DISTANCE;
	if (!passes) return;
	dragged = true;
	emit("toggle");
	stopDrag();
