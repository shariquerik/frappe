...
frappe.pages['translation-page'].on_page_load = function(wrapper) {
	...
	frappe.translation_page = new TranslationPage(page);
	$(wrapper).bind('show', function () {
		frappe.translation_page.create_split();
	});
}
...
class TranslationPage {
	constructor(page) {
		...
		this.wrapper.append(frappe.render_template('translation_page'));
		...
		let comments = [
			{
				...
				full_name: 'Albert Ritmeester',
				comment: 'The current approved translation is not correct, please review my translation:',
				time: '3 minutes ago'
			},
			{
				user: frappe.session.user,
				full_name: 'tukusejssirs',
				comment: 'I think that all glossary terms (even in English) should be in that basic form without any special capitalisation (only when the term is a name/title like Auto DevOps or GitLab). So, for example, this term should be ‘toggle’ rather than ‘Toggle’,',
				time: '6 minutes ago'
			},
			{
				user: frappe.session.user,
				full_name: 'Chad Walstrom',
				comment: 'Assuming this is a noun use.',
				time: 'a month ago'
			},
			{
				user: frappe.session.user,
				full_name: 'Paulo George Gomes Bezerra',
				comment: '@ToeOficial, can you give your opinion on this term? I thought "marcar com estrela", but it feels strange.',
				time: '3 months ago'
			},
			{
				user: frappe.session.user,
	...
	create_split() {
		this.layout = frappe.ui.Split(['.left-side', '.center-side', '.rightSide'], {
			sizes: [33, 34, 33],
			minSize: [250, 400, 250],
			snapOffset: 5,
			gutterSize: 4,
		...
		this.wrapper.find('.send-button').append(frappe.utils.icon('send', 'md'))
		this.wrapper.find('.comment-input textarea').attr('rows', '1');

		this.wrapper.find('.comment-input textarea').on('focus', () => {
			this.wrapper.find('.comment-input textarea').attr('rows', '5');
		})

		this.wrapper.find('.comment-input').on('focusout', () => {
			this.wrapper.find('.comment-input textarea').attr('rows', '1');
		})

		this.wrapper.closest('.layout-main-section-wrapper').css('margin-bottom', 'unset');
	}

	render_comments(comments) {
		let template = comment => `
			<div class="comment-1">
				<div class="comment-avatar">${frappe.avatar(comment.user, 'avatar-medium')}</div>
				<div class="comment-content">
					<div class="comment-user">${comment.full_name}</div>
					<div class="comment-description">${comment.comment}</div>
					<div class="comment-time text-muted">${comment.time}</div>
				</div>
			</div>
		`;

		let html = comments.map(template).join('');
		this.wrapper.find('.comment-area').html(html);
	}

	setup_language_filter() {
		this.languages = Object.keys(frappe.boot.lang_dict).map(language_label => {
		...
		let template = message => `
			<div
				...
				<div class="bold ellipsis">
					...
					<span class="ellipsis">${frappe.utils.escape_html(message.source_text)}</span>
				</div>
			</div>
		`;
	...
	setup_positions(positions) {
		...
		if (positions && positions.length) {
			position_dom = positions.map(position => {
				if (position && !position.path) return;
