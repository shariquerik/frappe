...
- **Read + name + avatar** ride **own-user permission**: a user may read/write their *own* `User`
  document via `User.has_website_permission` (`self.name == frappe.session.user`), with
  **permlevel** still gating fields — `first_name` / `last_name` / `user_image` are permlevel 0
  (self-writable); `roles` / `user_type` / `enabled` / API keys are not. So the composable wraps
  `createDocumentResource('User', <self>)` and saves only those permlevel-0 fields. **No
...
takes for an own-user doc — the **website-permission** path is — which is why self-service works
for non-admins without weakening the doctype.
