...
function onKey(e: KeyboardEvent) {
  if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); os.state.paletteOpen = !os.state.paletteOpen; os.state.paletteQuery = ''; os.state.menu = null }
  if (e.key === 'Escape') { os.state.paletteOpen = false; os.state.menu = null; os.state.wpPicker = false }
}
...
<template>
  ...
  <div :ref="(el) => os.setDeskEl(el as HTMLElement | null)" :data-theme="os.state.theme" :data-active-window="os.state.activeId || ''" class="relative h-screen w-full overflow-hidden bg-surface-gray-3 text-ink-gray-8 [font-family:var(--font-sans)] isolate">
    <!-- wallpaper -->
    <div class="absolute inset-0 z-0" :style="{ background: wp.bg }" @contextmenu.prevent="os.state.wpPicker = true"></div>

    <!-- desktop icons -->
    <div class="absolute right-[18px] top-[46px] z-[1] flex flex-col gap-[14px]">
      <button v-for="(di, i) in desktopIcons" :key="i" class="flex w-[76px] cursor-pointer flex-col items-center gap-[5px] rounded-lg border-none bg-transparent px-0.5 py-1.5 hover:bg-[var(--surface-alpha-white-3)]" @click="di.open()">
        <span class="inline-flex h-[46px] w-[46px] items-center justify-center rounded-[11px] border border-outline-gray-2 bg-surface-base text-ink-gray-6 shadow-[var(--shadow-sm)]">
    ...
    <!-- empty hint -->
    <div v-if="noWindows" class="pointer-events-none absolute inset-0 z-0 flex flex-col items-center justify-center gap-3" :style="wp.dark ? 'color:rgba(255,255,255,0.78);text-shadow:0 1px 3px rgba(0,0,0,0.4);' : 'color:var(--ink-gray-5);'">
      <span class="lucide-app-window size-[44px]"></span>
      <div class="max-w-[280px] text-center text-[13.5px] leading-normal">Open an app from the dock below to get started.</div>
    </div>
    ...
    <!-- split exit pill -->
    <button v-if="os.state.split" class="absolute left-1/2 top-[42px] z-[80] inline-flex h-[30px] -translate-x-1/2 cursor-pointer items-center gap-[7px] rounded-full border border-outline-gray-2 bg-surface-base px-[14px] text-[12px] font-medium text-ink-gray-7 shadow-[var(--shadow-lg)]" @click="os.exitSplit()">
    ...
    <!-- Popover layer: reka-ui popovers (Dropdown, Select, …) portal here so
         ...
         command palette (95000), which is a modal that should win. -->
