...
const fieldMeta = computed(() =>
	doctype.value ? os.fieldMetaFor(doctype.value) : { loading: false, data: null, error: null },
);
watch(
	doctype,
	(dt) => {
...
watch(
	[doctype, () => props.recordName, isNew],
	() => {
		if (!isNew.value && doctype.value && props.recordName)
			os.loadDoc(doctype.value, props.recordName);
	},
	{ immediate: true },
);
...
const singleTab = computed(
	() => (layout.value ?? []).filter((tab: any) => !tab.hidden).length <= 1,
);
...
<template>
	<div class="flex h-full flex-col">
		...
		<div
			v-else
			class="flex flex-shrink-0 items-center gap-2.5 border-b border-outline-gray-1 px-4 py-[11px]"
		>
		...
		<div
			v-if="saveError"
			class="flex-shrink-0 border-b border-outline-red-2 bg-surface-red-1 px-4 py-2 text-[12px] text-ink-red-6"
		>
			{{ saveError }}
		</div>
		...
		<div class="min-h-0 flex-1 overflow-auto">
			<div
				v-if="fieldMeta.error || layoutError"
				class="px-5 py-5 text-[13px] text-ink-red-6"
			>
				{{ fieldMeta.error || String(layoutError) }}
			</div>
			<div v-else-if="!isNew && docState.error" class="px-5 py-5 text-[13px] text-ink-red-6">
				{{ docState.error }}
			</div>
			<div
				v-else-if="fieldMeta.loading || layoutLoading || (!isNew && docState.loading)"
				class="px-5 py-5 text-[13px] text-ink-gray-4"
			>
				Loading…
			</div>
			<FormLayout
				...
				:class="{ '[&_[role=\'tablist\']]:hidden': singleTab }"
			/>
		</div>
	</div>
</template>
